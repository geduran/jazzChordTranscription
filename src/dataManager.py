...
#                curr_feature = audioData.norm_data(np.abs(audioData.norm_data(curr_feature)), 'zero_one').T
    ...
    def get_multipitch(self, audioData, win_len=4096, hop_len=1024, HPSS=False):
        ...
        pos = (pos * hop_len + win_len/hop_len)/audioData.sr
...
#                                              67, 71, 74, 77]], :] = 0
    ...
    def bass_cnn_segmentation(self, audio_data, model, mode='all', midi=0):
        ...
        #audio_data.name = 'AliceInWonderland'
        #mode = 'all'
        ...
        #print('hop_len: {}. peaks: {}'.format(audio_data.hop_len, peaks))
        return (peaks * audio_data.hop_len +
                          audio_data.win_len/audio_data.hop_len)/audio_data.sr

    def bass_rnn_segmentation(self, audio_data, model, mode='all', midi=0):
...
class BassOnsetDetector(BassManager):
    """
    Inherits from BassManager, but only performs Bass onsetDetection, do
    not include testings and performance evaluation.
    """
    ...
    def get_cnn_audio_data(self, audio):
        sr = 44100
        win_len = 4096
        hop_len = 256

        cnn_audio_data = AudioData([],audio=audio, win_len=win_len, hop_len=hop_len,
                                   HPSS=False, get_features=False)
        # cnn_audio_data.get_cnn_features(sr=sr, win_len=win_len, hop_len=hop_len)
        return cnn_audio_data

    def get_rnn_audio_data(self, audio):
        sr = 44100
        win_len = 4096
        hop_len = 256

        rnn_audio_data = AudioData([], audio=audio, win_len=win_len, hop_len=hop_len,
                                   HPSS=False, get_features=False)
        rnn_audio_data.get_rnn_features(sr=sr, win_len=win_len, hop_len=hop_len)

        return rnn_audio_data


    def cnn_onset_detect(self, audio):
        cnn_onsets = []
        if audio is list:
            for audio_seg in audio:
                audio_data = self.get_cnn_audio_data(audio_seg)
                cnn_onsets.append(self.bass_cnn_segmentation(audio_data,
                                                            self.cnn_model))
        else:
            if type(audio).__module__ == np.__name__:
                audio_data = self.get_cnn_audio_data(audio)
            else:
                audio_data = audio
            cnn_onsets = self.bass_cnn_segmentation(audio_data, self.cnn_model)
        return cnn_onsets

    def rnn_onset_detect(self, audio):
        # print(len(audio))
        rnn_onsets = []
        if isinstance(audio, collections.Iterable):
            for audio_seg in audio:
                audio_data = self.get_rnn_audio_data(audio_seg)
                rnn_onsets.append(self.bass_rnn_segmentation(audio_data,
                                                            self.rnn_model))
        else:
            if type(audio).__module__ == np.__name__:
                audio_data = self.get_rnn_audio_data(audio)
            else:
                audio_data = audio
            rnn_onsets = self.bass_rnn_segmentation(audio_data, self.rnn_model)
        return rnn_onsets
