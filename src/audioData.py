...
class Features:
    ...
    """
    def __init__(self, win_len=None, hop_len=None):
        self.mfcc = None
        self.mfcc_flux = None
        self.chroma = None
...
class AudioData:
    ...
    def __init__(self, path, audio=[], win_len=None, hop_len=None, HPSS=True,
                 only_bass=False, get_features=True):
        if len(path):
            self.path = path
        ...
        self.audio_p = None
        if get_features:
            self.get_features(HPSS=HPSS,
                              only_bass=only_bass)
        #self.save()


    def get_features(self,HPSS=False, only_bass=False):
        feature = Features(self.win_len, self.hop_len)
        print('\n')
        ...
        S_h, _ = librosa.magphase(librosa.core.stft(y=self.audio_h,
                                n_fft=self.win_len, hop_length=self.hop_len))
        S = S_h
        ...
        feature.rms = librosa.feature.rmse(S=S_p, frame_length=self.win_len,
                                           hop_length=self.hop_len)
    ...
    @staticmethod
    def getCQT(audio, hop_len=1024, n_bins=84, bins_per_octave=12, sr=44100,
               tuning=0.0):

        C = np.abs(librosa.cqt(audio, hop_length=hop_len, n_bins=n_bins,
                              bins_per_octave=bins_per_octave, sr=sr,
                              tuning=tuning))
        return C
    ...
    def get_cnn_features(self, sr=44100, win_len=4096, hop_len=256):

        cnn_audio_features = Features(win_len=win_len, hop_len=hop_len)
        audioFrame = madmom.audio.signal.FramedSignal(self.audio,
                                                      frame_size=win_len,
                                                      hop_size=hop_len,
                                                      sample_rate=sr)
        mel1 = madmom.audio.FilteredSpectrogram(audioFrame,
                                        filterbank=MelFilterbank, num_bands=39,
                                        fmin=40, fmax=2000)

        audioFrame = madmom.audio.signal.FramedSignal(self.audio,
                                                      frame_size=win_len/2,
                                                      hop_size=hop_len,
                                                      sample_rate=sr)
        mel2 = madmom.audio.FilteredSpectrogram(audioFrame,
                                        filterbank=MelFilterbank, num_bands=39,
                                        fmin=40, fmax=2000)

        audioFrame = madmom.audio.signal.FramedSignal(self.audio,
                                                      frame_size=win_len/4,
                                                      hop_size=hop_len,
                                                      sample_rate=sr)
        mel3 = madmom.audio.FilteredSpectrogram(audioFrame,
                                        filterbank=MelFilterbank, num_bands=50,
                                        fmin=40, fmax=2000)

        cnn_audio_features.bass_mel_spectrogram1_cnn = mel1
        cnn_audio_features.bass_mel_spectrogram2_cnn = mel2
        cnn_audio_features.bass_mel_spectrogram3_cnn = mel3

        self.features = cnn_audio_features

    def get_rnn_features(self, sr=44100, win_len=4096, hop_len=256):

        rnn_audio_features = Features(win_len=win_len, hop_len=hop_len)
        audioFrame = madmom.audio.signal.FramedSignal(self.audio,
                                                      frame_size=win_len,
                                                      hop_size=hop_len,
                                                      sample_rate=sr)
        ...
        rnn_audio_features.bass_mel_spectrogram1 = mel1
        rnn_audio_features.bass_mel_spectrogram2 = mel2
        rnn_audio_features.bass_mel_spectrogram3 = mel3

        self.features = rnn_audio_features
